...
<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm p-4">
                <h2 class="mb-2 text-center">{{ $artikel->judul }}</h2>
                <p class="text-muted text-center" style="font-size: 0.95rem;">{{ $artikel->created_at->format('d-m-Y') }}</p>
                @if($artikel->gambar)
                    <div class="text-center mb-4">
                        <img src="{{ asset('storage/'.$artikel->gambar) }}" alt="Gambar Artikel" class="img-fluid rounded" style="max-height:350px; object-fit:cover;">
                    </div>
                @endif
    ...
    @if(isset($artikels_lain) && $artikels_lain->count())
    <div class="row justify-content-center mt-5">
        <div class="col-lg-10">
            <h4 class="mb-4 fw-bold">Baca Juga</h4>
            <div class="row">
                @foreach($artikels_lain as $lain)
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        @if($lain->gambar)
                        <img src="{{ asset('storage/'.$lain->gambar) }}" alt="Banner Artikel" class="card-img-top" style="height:180px; object-fit:cover;">
                        @endif
                        <div class="card-body">
                            <h6 class="card-title fw-bold" style="min-height:48px;">
                                <a href="{{ route('artikel.show', $lain->id) }}" style="color:inherit; text-decoration:none;">{{ $lain->judul }}</a>
                            </h6>
