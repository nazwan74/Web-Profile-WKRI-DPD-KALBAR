...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item mx-2">
                        <a class="nav-link active" href="{{ route('artikel.list') }}">Artikel</a>
                    </li>
                    <li class="nav-item mx-2">
    ...
    <div class="container my-5">
        <h2 class="mb-4 fw-bold text-center">Daftar Artikel</h2>
        <div class="row">
            @forelse($artikels as $artikel)
            <div class="col-md-4 mb-4">
                <div class="card h-100 shadow-sm card-anim">
                    @if($artikel->gambar)
                    <img src="{{ asset('storage/'.$artikel->gambar) }}" alt="Banner Artikel" class="card-img-top" style="height:180px; object-fit:cover;">
                    @endif
                    <div class="card-body">
                        <h6 class="card-title fw-bold" style="min-height:48px;">
                            <a href="{{ route('artikel.show', $artikel->id) }}" style="color:inherit; text-decoration:none;">{{ $artikel->judul }}</a>
                        </h6>
                        <p class="text-muted mb-1" style="font-size:0.95rem;">{{ $artikel->created_at->format('d-m-Y') }}</p>
                        <div style="font-size:0.98rem; color:#444; min-height:60px;">
                            {{ strip_tags(Str::limit($artikel->isi, 100)) }}
                        </div>
                        <a href="{{ route('artikel.show', $artikel->id) }}" class="btn btn-sm btn-primary mt-2">Baca Selengkapnya</a>
                    </div>
                </div>
            </div>
            @empty
